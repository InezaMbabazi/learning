...
def schedule_rooms(assignments, room_df):
    ...
    for _, row in assigned.iterrows():
        ...
        sessions_needed = 2  # Fixed: 2 sessions per module group
        ...
        for slot in shuffled_slots:
            for day in shuffled_days:
                ...
                # Optional 1-day break rule - currently disabled (uncomment to enable)
                # if scheduled_days:
    ...
    if show_reassign:
        ...
        if st.button("🔁 Apply Reassignments"):
            ...
            try:
                ...
                # Instead of experimental_rerun, just update display below
                # st.experimental_rerun()
            except Exception as e:
                error = str(e)
            if error:
                st.error(f"Error applying reassignment: {error}")
    ...
    if unassigned_modules:
        ...
        for item in unassigned_modules:
            st.write(f"Module: {item['Module']}, Group: {item['Group']}, Lecturer: {item['Lecturer']}, "
                     f"Students: {item['Students']}, Sessions Scheduled: {item['Sessions Scheduled']}/{item['Sessions Needed']}")
