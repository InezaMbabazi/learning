...
if teacher_file and module_file:
    ...
    for _, module in modules_df.iterrows():
        ...
        # Find teachers eligible for assignment
        ...
        if not eligible_teachers.empty:
            ...
            module['Assigned Teacher'] = teacher["Teacher's Name"]
            assigned = True

            # Assign assistant teacher for large classes (if any)
            if module['Class Size'] > 50:
                ...
                if not assistant_candidates.empty:
                    ...
                    module['Assistant Teacher'] = assistant_teacher["Teacher's Name"]
                    teachers_df.loc[assistant_teacher.name, 'Weekly Assigned Hours'] += module['Total Weekly Hours']

        if not assigned:
            # If no teacher is available, log module as unassigned
            module['Assigned Teacher'] = 'Unassigned'
    ...
    for teacher_name, teacher_data in teachers_df.iterrows():
        if teacher_data['Weekly Assigned Hours'] > 12:
            ...
            for _, module in modules_df[modules_df['Assigned Teacher'] == teacher_name].iterrows():
                if excess_hours <= 0:
                    break
                if module['Total Weekly Hours'] <= excess_hours:
                    ...
                    if not eligible_teachers.empty:
                        ...
                        module['Assigned Teacher'] = new_teacher["Teacher's Name"]
